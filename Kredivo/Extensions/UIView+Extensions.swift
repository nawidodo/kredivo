...
extension UIView {
    ...
    public static func getView(with identifier: String) -> UIView? {
        if let _ = Bundle.main.path(forResource: identifier, ofType: "nib"),
            ...
            return tempView
        } else if let customviewClass = self.getClass(from: identifier), let viewType = customviewClass as? UIView.Type {
    ...
    public static func getClass(from className: String) -> AnyClass? {
        if let bundleName: String = Bundle.main.infoDictionary?["CFBundleExecutable"] as? String,
